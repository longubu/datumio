...
def main(X, y, BATCH_SIZE=32, axis=0, show_aug_test=True):
    ...
    Batcher = dtd.BatchGenerator(X, y=y, batch_size=BATCH_SIZE, shuffle=False)
    ...
    Batcher = dtd.BatchGenerator(X, y=y, batch_size=BATCH_SIZE, shuffle=True)
    batchgen = Batcher.get_batch(chw_order=False, dtype=np.uint8)
    ...
    Batcher = dtd.BatchGenerator(X, batch_size=BATCH_SIZE, shuffle=False)
    ...
    Batcher = dtd.BatchGenerator(X, y=y, dataset_zmuv=True, dataset_axis=axis,
    ...
    Batcher = dtd.BatchGenerator(X, y=y, batch_zmuv=True, batch_axis=axis,
                                 batch_size=BATCH_SIZE, shuffle=False)
    mb_x, mb_y = Batcher.get_batch(chw_order=False, dtype=np.float32).next()
    ...
    Batcher = dtd.BatchGenerator(X, y=y, sample_zmuv=True, sample_axis=axis,
                                 batch_size=BATCH_SIZE, shuffle=False)
    mb_x, mb_y = Batcher.get_batch(chw_order=False, dtype=np.float32).next()
...
if __name__ == '__main__':
    ...
    main(X, y, BATCH_SIZE=32, axis=0, show_aug_test=True)
